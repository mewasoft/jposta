const a = (o, s, t) => {
  const i = o[s];
  return i ? typeof i == "function" ? i() : Promise.resolve(i) : new Promise((p, z) => {
    (typeof queueMicrotask == "function" ? queueMicrotask : setTimeout)(z.bind(null, new Error("Unknown variable dynamic import: " + s + (s.split("/").length !== t ? ". Note that variables only represent file names one level deep." : ""))));
  });
}, j = {
  host: ""
}, m = j, e = [
  ...
  if (o.length === 7 && !/^\d{7}$/.test(o))
    throw new Error(`Invalid zip code: ${o}`);
  const s = o.replace("-", ""), t = s.slice(0, 2), i = await c(t);
  if (!i || !i[s])
    return null;
  const [p, z, n] = i[s], r = e[p - 1];
  if (typeof p != "number" || typeof z != "string" || typeof n != "string" || typeof r != "string")
  ...
  if (m.host !== "") {
    const { default: t } = await import(`${m.host}/z${o}.json`);
    return t;
  }
