const f = (o, i, t) => {
  const s = o[i];
  return s ? typeof s == "function" ? s() : Promise.resolve(s) : new Promise((p, r) => {
    (typeof queueMicrotask == "function" ? queueMicrotask : setTimeout)(r.bind(null, new Error("Unknown variable dynamic import: " + i + (i.split("/").length !== t ? ". Note that variables only represent file names one level deep." : ""))));
  });
}, e = {
  host: ""
}, j = e, c = [
  ...
  if (o.length === 7 && !/^\d{7}$/.test(o))
    throw new Error(`Invalid zip code: ${o}`);
  const i = o.replace("-", ""), t = i.slice(0, 2), s = await a(t);
  if (!s || !s[i])
    return null;
  const [p, r, m, z] = s[i], n = c[p - 1];
  if (typeof p != "number" || typeof r != "number" || typeof m != "string" || typeof z != "string" || typeof n != "string")
    throw new Error(`Internal error data broken: ${s[i]}`);
  return {
    pref: n,
    prefNum: p,
    cityCode: r,
  ...
  for (let t = 0; t <= 99; t++) {
    ...
    try {
      ...
      if (!p)
        continue;
      for (const [, r] of Object.entries(p)) {
        const [m, z, n] = r;
        m === o && n && !i.has(z) && i.set(z, n);
      }
    } catch {
      continue;
    }
  }
  return Array.from(i.entries()).map(([t, s]) => ({ key: t, name: s })).sort((t, s) => t.key - s.key);
};
export {
  l as configureJposta,
  u as getAddress,
  g as getCitiesByPref,
  y as getPrefs
};
